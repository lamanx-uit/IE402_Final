...
export const loadBackWallDecor = (scene) => {
  const loader = new GLTFLoader();

  // Back wall is at Z = 12.5
  const wallZ = 12;
  ...
  // Load curtain_right model
  loader.load("../public/models/curtain_right.glb", (gltf) => {
    curtainModel = gltf.scene;
    console.log("Curtain Right Loaded", gltf);
    curtainsLoaded = true;
  ...
  function placeItems() {
    ...
    pattern.forEach((itemType, index) => {
      ...
      if (itemType === 'curtain') {
        ...
        curtain.position.set(centerX, 5, wallZ);
        curtain.rotation.set(0, Math.PI / 2, 0); // Rotate to face into the room from back wall
        curtain.scale.set(13.75, 20.5, 13.75);
        scene.add(curtain);
        ...
        glass.position.set(centerX, 0, wallZ);
        glass.rotation.set(0, Math.PI, 0); // Rotate to face into room from back wall
        glass.scale.set(3, 13.5, 3);
        scene.add(glass);
    ...
    light1.rotation.set(0, Math.PI / 2, 0); // Rotate for back wall
    ...
    [2, 4, 6].forEach(idx => {
      ...
      light.rotation.set(0, Math.PI / 2, 0); // Rotate for back wall
    ...
    light5.rotation.set(0, Math.PI / 2, 0); // Rotate for back wall
